...
def pipeline_rna_count(sample_sequences, vocab):
    fragmenter = KmerFragmenter()
    rng = np.random.seed(42)
    fragments = fragmenter.split_words(sample_sequences, 3, 3)
    vectorizer = CountVectorizer(vocabulary=vocab)
    vectors = vectorizer.fit_transform([" ".join(kmer_list) for kmer_list in fragments])
    return vectors
